...
def main(n: int = 4):
    ...
    for i, x in enumerate(items):
        ...
        log.info(f"Generando embedding para documento {i+1}/{len(items)}")
    ...
    # 5) Upload documentos completos (sin chunking)
    uploaded_docs = []
    for i, x in enumerate(items):
        ...
        log.info(f"Subiendo documento completo {i+1}/{len(items)}: {doc_name[:50]}...")
        
        # 5.a) Primero crear el documento en RAGFlow
        ...
        # 5.b) Luego agregar el chunk completo (sin dividir) con embedding
        chunk_result = rf_add_complete_chunk(
            dataset_id=dataset_id,
            document_id=doc_id,
            content_text=content_text,
            metadata=meta,
        ...
        log.info(f"✓ Documento {i+1} subido exitosamente como documento completo")

    log.info(f"✅ Proceso completado. {len(uploaded_docs)} documentos subidos como documentos completos (sin chunking)")

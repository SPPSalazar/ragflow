...
def rf_upload_text_document(dataset_id: str, name: str, text: str) -> str:
    ...
    # Algunos builds usan /files; si 404, prueba:
    # url = f"{RAGFLOW_BASE_URL}/api/v1/datasets/{dataset_id}/files"

    files = {
        # nombre de campo debe ser "file"
        "file": (f"{name or 'documento'}.txt", text.encode("utf-8"), "text/plain")
    }
    data = {
        # meta opcional; varios builds aceptan name/metadata aquí
        "name": name
    }
    headers = {"Authorization": f"Bearer {RAGFLOW_API_KEY}"}  # NO pongas Content-Type manual
    ...
    # Ajusta según tu payload; típicamente viene en data.id o data.document.id
    data_obj = jr.get("data") or {}

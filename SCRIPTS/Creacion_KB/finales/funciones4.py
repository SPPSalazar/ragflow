...
# =========================
def sample_noticias_structured(year) -> List[Dict[str, Any]]:
...
# Muestreo y transformación
# =========================
def sample_last3months() -> List[Dict[str, Any]]:
  client = _get_mongo_client()
  db = client[MONGO_DB]
  col = db[MONGO_COL]
  hoy = datetime.now()
...
def sample_noticias_3months_complemento() -> List[Dict[str, Any]]:
    client = _get_mongo_client()
    db = client[MONGO_DB]
    col = db[MONGO_COL]

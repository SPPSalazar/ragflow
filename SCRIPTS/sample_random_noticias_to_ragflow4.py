...
RAGFLOW_CHUNK_METHOD  = os.getenv("RAGFLOW_CHUNK_METHOD", "none")
...
def rf_upload_json_document(dataset_id: str, name: str, content_text: str, metadata: Dict[str, Any]) -> str:
    ...
    *El doc_id real lo devuelve el servidor en la respuesta.*
    ...
    files = {
        # Algunas builds requieren text/plain; si ves error, cambia a "text/plain"
...
def main(n: int = 3):
    ...
    # 5) Upload JSON file + agregar chunk con metadata
    for x in items:
        meta = x["metadata"].copy()
        doc_name = meta.get("titulo") or meta.get("url") or meta.get("id_original") or "noticia"

        # 5.a Subir archivo JSON con content + metadata (para trazabilidad en UI)
        ...
        # 5.b Insertar el chunk “real” (texto+metadata+embedding) en Infinity
        _ = rf_add_chunk(
            dataset_id=dataset_id,
            document_id=doc_id,
            content_text=x["content"]["contenido"] or "",
